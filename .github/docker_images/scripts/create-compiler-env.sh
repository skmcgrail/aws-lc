...
SEARCH_PATHS=("/usr/bin" "/usr/local/bin" "/opt/bin" "/bin")
    ...
    for search_path in "${SEARCH_PATHS[@]}"; do
        if [[ -d "$search_path" ]]; then
            ...
            while IFS= read -r -d '' compiler_path; do
                local basename_comp=$(basename "$compiler_path")
            ...
            done < <(find "$search_path" -maxdepth 1 -name "${compiler_base}-[0-9]*" -type f -executable -print0 2>/dev/null)
        fi
    done
...
main() {
    ...
    echo "Usage: source /opt/compiler-env/setup-<compiler>.sh"
}
